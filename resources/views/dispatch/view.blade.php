				...
				<div class="card">
					<div class="card-body">
						<form id="form" method="POST" action="{{config('app.baseURL')}}/checking/order-status/{{$detail->id}}">
							@csrf
							<p  align="right">
							<button type="submit" @if($detail->status==3) disabled @endif class="btn btn-success"><i class="bx bx-check"></i> Mark As Job Done</button>&nbsp;&nbsp;&nbsp;&nbsp;<button id="addBtn" @if($detail->status==3) disabled @endif onclick="callTr()" class="btn btn-primary"><i class="bx bx-plus"></i></button></p>
